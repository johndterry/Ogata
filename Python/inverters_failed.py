...
class AdOg:
    ...
    def compare(self, w, b_min_half, b_max_dub,nu,storage,peak,bc,qT,ib,it,h,N):
        ...
        zero1 = jn_zeros(nu, 1)[0]
        #xi0=zero1/np.pi
        #xin0= jn_zeros(nu,N)[-1]/np.pi
        xin20=jn_zeros(nu,N)[-1]/np.pi
        #val1=abs(w(np.pi/h1*get_psi(h1*xi0))*jv(nu,np.pi/h1*get_psi(h1*xi0))*get_psip(np.pi/h1*get_psi(h1*xi0)))+abs(w(np.pi/h1*get_psi(h1*xin0))*jv(nu,np.pi/h1*get_psi(h1*xin0))*get_psip(np.pi/h1*get_psi(h1*xin0)))
        #val2=abs(w(np.pi/h0*get_psi(h1*xi0))*jv(nu,np.pi/h0*get_psi(h1*xi0))*get_psip(np.pi/h0*get_psi(h0*xi0)))+abs(w(np.pi/h0*get_psi(h0*xin20))*jv(nu,np.pi/h0*get_psi(h0*xin20))*get_psip(np.pi/h0*get_psi(h0*xin20)))
        #xin0= jn_zeros(nu,N)[  -1]/np.pi
        #xin20=jn_zeros(nu,2*N)[-1]/np.pi
        #val1=abs(w(np.pi/h1*get_psi(h1*xin20))*jv(nu,np.pi/h1*get_psi(h1*xin20))*get_psip(np.pi/h1*get_psi(h1*xin20)))
        #val2=abs(w(np.pi/h0*get_psi(h0*xin20))*jv(nu,np.pi/h0*get_psi(h0*xin20))*get_psip(np.pi/h0*get_psi(h0*xin20)))
        #val1=abs(w(np.pi*xin20)*jv(nu,np.pi/h1*get_psi(h1*xin20)))
        #val2=abs(w(np.pi*xin20)*jv(nu,np.pi/h0*get_psi(h*xin20)))
        if ib+it== 2:
            try:
                top = stor[b_max_dub]
            except:
            ...
            if bot > peak:
                ...
                peakval = peak
                bcc = bc
        #bot=abs(w(np.pi/h1*get_psi(h1*zero1/np.pi)))
        #top=abs(w(np.pi/h0*get_psi(h0*xin20/2)))
        peakval = peak
        bcc = bc
        #if h> 0.03:
        #    return True,storage,peakval,bcc
        #else:
            #print h>2*np.exp(-np.pi/2*np.exp(h*N))*w(np.pi*N)
        #    print 2*np.exp(-np.pi/2*np.exp(h*N))*w(np.pi*N*(1.0-2*np.exp(-np.pi/2*np.exp(h*N))))
        #    return h*np.exp(-np.pi**2/2/h)>2*np.exp(-np.pi/2*np.exp(h*N))*w(np.pi*N*(1.0-2*np.exp(-np.pi/2*np.exp(h*N)))),storage,peakval,bcc
        #print 2*np.exp(-np.pi/2*np.exp(h*N))*w(np.pi*N*(1.0-2*np.exp(-np.pi/2*np.exp(h*N))))
        h1=self.get_h(nu,h,N)
        val1=h1*np.exp(-np.pi**2/2/h1)+abs(w(np.pi/h1*get_psi(h1*xin20))*jv(nu,np.pi/h1*get_psi(h1*xin20))*get_psip(np.pi/h1*get_psi(h1*xin20)))
        val2=h *np.exp(-np.pi**2/2/h )+abs(w(np.pi/h *get_psi(h1*xin20))*jv(nu,np.pi/h *get_psi(h *xin20))*get_psip(np.pi/h *get_psi(h *xin20)))
        return val1<val2,storage,peakval,bcc#h*np.exp(-np.pi**2/2/h)+2*np.exp(-np.pi/2*np.exp(h*N))*w(np.pi*N*(1.0-2*np.exp(-np.pi/2*np.exp(h*N))))>2*np.exp(-np.pi/2*np.exp(h*N))*w(np.pi*N*(1.0-2*np.exp(-np.pi/2*np.exp(h*N)))),storage,peakval,bcc
        #    print val1<val2
        #    return (b_min_half/np.pi/zero1**2*qT)*bot>top,storage,peakval,bcc#val1<val2,storage,peakval,bcc#h*bot>top,storage,peakval,bcc#
